...
function generatePdf(download, process, candidate, pageNumber, pageBreak) {
  const warnSizes = 9;
  ...
  let dd = [
          ...
          [
            {
              columns: [
                { text: process },
                { text: `AREA ${candidate.area}`, alignment: "right" },
              ],
            },
          ],
          [
    ...
    "-----------------------------------------------------------------------------------------------------------------------------------------------------------",
    {
      table: {
        ...
        body: [
          [
            { text: "Concurso: ETC ETC - EDITAL XX/20XX UNIFAP" },
            { text: `AREA ${candidate.area}` },
          ],
          [
            {
    ...
    " ",
    {
      ...
      pageBreak: pageBreak ? "after" : false,
    },
    // ],
  ];
  //   return dd;
  if (download) pdfMake.createPdf({ content: dd }).download("prova.pdf");
  else return dd;
}
